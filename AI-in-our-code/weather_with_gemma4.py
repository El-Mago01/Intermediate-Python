...
# Define the model you downloaded (Gemma 4 e4b)
MODEL_NAME = 'gemma4:e2b'

# Initialize a streaming chat request
def ask_gemma(prompt)->dict:
    print("This is the prompt that gemma gets:", prompt)
    response_stream = ollama.chat(
        model=MODEL_NAME,
        messages=[
            {
                'role': 'expert in clothing advice with good sense of humor and Amsterdam Ajax fan',
                'content': prompt,
            },
        ],
        stream=True,
...
# Print the incoming text blocks in real time
...
def get_meta_prompt(weather_params: dict, sex, birth_year, city) -> str:
    meta_prompt=[]
    meta_prompt.append(textwrap.dedent(f"""
        You are an advisor for dressing for going out doors. 
        Based upon specific weather parameters, sex, and age you will provide
        ...
        the sex and age."""))
    meta_prompt.append(f"The user is a {sex} and is born in {birth_year}\n")
    meta_prompt.append(f"The city is : {city}\n")
    meta_prompt.append("The weather parameters for the coming hour are as follows:\n")
    for weather_param in weather_params.keys():
        meta_prompt.append(weather_param + " : " + str(weather_params[weather_param]) + "\n")

    meta_prompt.append(textwrap.dedent("""You first start to shortly state what the weather is 
        like for the next hours, based upon the provided parameters. In this 
        initial words, you will also indicate the city you're giving advice for..
        ...
        Make sure you tell a different joke every time.
        You can use maximum use 300 words"""))
    prompt = "\n".join(meta_prompt)
    return prompt

def pretty_print(advice):
    print(type(advice))
    print("===============================================================")
